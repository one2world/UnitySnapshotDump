...
namespace Unity.MemoryProfiler.Editor
{
    ...
    internal class CachedSnapshot : IDisposable
    {
        ...
        public class ManagedMemorySectionEntriesCache : IDisposable
        {
            ...
            static readonly string k_VMSection = "Virtual Machine Memory Section";
            static readonly string k_GCSection = "Managed Heap Section";
            static readonly string k_ActiveGCSection = "Active Managed Heap Section";
            static readonly string k_StackSection = "Managed Stack Section";
            static readonly string k_ManagedMemorySection = "Managed Memory Section (unclear if Heap or Virtual Machine memory, please update Unity)";
            ...
            for (int i = 0; i < elements.Length; ++i)
            {
                ...
                else
                {
                    ...
                    else if (typeof(T) == typeof(int[]))
                    {
                        ...
                        srcSpan.CopyTo(System.Runtime.InteropServices.MemoryMarshal.Cast<int, byte>(new Span<int>(bytes)));
                        arr = bytes;
                    }
                    else if (typeof(T) == typeof(ulong[]))
                    {
                        var bytes = new ulong[actualLength / UnsafeUtility.SizeOf<ulong>()];
                        srcSpan.CopyTo(System.Runtime.InteropServices.MemoryMarshal.Cast<ulong, byte>(new Span<ulong>(bytes)));
                        arr = bytes;
                    }
                    else if (typeof(T) == typeof(long[]))
                    {
                        var bytes = new long[actualLength / UnsafeUtility.SizeOf<long>()];
                        srcSpan.CopyTo(System.Runtime.InteropServices.MemoryMarshal.Cast<long, byte>(new Span<long>(bytes)));
                        arr = bytes;
                    }
                    else
                    {
